...
> This repository is now in **closeout verification**.

> The authoritative CI, security, contributor, and public documentation posture has moved to the main [`Elegy`](../Elegy) monorepo.

> Use Elegy for active work and treat this repository as a temporary verification surface pending final deletion.
...
- a Rust-native contract consumer layer for the governed Elegy skill/discovery bundle:
   ...
   - semantic validation for origin/governance-sensitive skill rules


   - semantic validation for MCP descriptor and analysis consistency


   - support-manifest and compatibility-manifest conformance checks against exported Elegy artifacts
...
## Relationship to the broader Elegy ecosystem





`Elegy-MCP` now exists only as a closeout-verification snapshot while the active MCP runtime family has been consolidated into the main `Elegy` monorepo.





It is not the umbrella home for every Elegy subsystem.





The broader package and contract ecosystem can live in an umbrella `Elegy` repo, while this repository stays narrow and strong at:





- descriptor loading and normalization for the MCP runtime slice


- policy-bounded runtime composition


- adapter-driven resource support


- stdio MCP hosting and related runtime surfaces
...
- general skill-core contracts should not be re-authored in this repository; when they are needed, they should be consumed through exported Elegy artifacts and Rust-native models rather than through .NET runtime code


- generated-tooling or scaffolding concerns should stay as peer ecosystem concerns rather than broadening this repo beyond its runtime-composition focus


- the `elegy` CLI here is the operator shell for this runtime surface, not the definition of every CLI-generation concern across the ecosystem





This boundary helps keep `Elegy-MCP` reusable, auditable, and language-appropriate without turning it into a catch-all framework.
